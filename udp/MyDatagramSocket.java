...
public class MyDatagramSocket extends DatagramSocket {
	...
	private static final int _PROB_MSG_LOSS = 100; // 10% packet loss
	...
	private static final double _PPT_JITTER = 0.10; // factor de jitter no tempo
													// de propagacao
	private static final double _PPT_AVERAGE = 150; // tempo de propagacao medio
													// em mili segundos
				...
				for (;;) {
					synchronized (queue) {
						try {
							...
							if (!MyDatagramSocket.super.isClosed())
